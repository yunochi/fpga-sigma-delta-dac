module sigma_delta_3rd_order #(
    ...
    wire signed [WIDTH-1:0] din_ext = $signed(din) <<< 8;
    ...
    wire signed [WIDTH-1:0] reduced_din = (din_ext * 26) >>> 5;
    ...
    wire signed [WIDTH+3:0] sum = integrator1 + (integrator2 >>> 2) + (integrator3 >>> 4) + (integrator3 >>> 6);
