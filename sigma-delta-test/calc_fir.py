...
def design_halfband_filter(taps=127):
...
# 필터 설계 실행 및 출력
taps = 127
coefs = design_halfband_filter(taps=taps)
center_idx = (taps - 1) // 2

print(f"Designed {len(coefs)}-tap Half-Band FIR Coefficients (center at idx {center_idx}):")
print(coefs)
print()

# Verilog get_coef() 용으로 outermost -> innermost 순서로 출력
# (Verilog 코드에서 idx=0 이 outermost, idx 끝이 innermost)
side_indices = [i for i in range(center_idx + 1, taps) if (i - center_idx) % 2 != 0]
# -> 뒤집어서 outermost 먼저 출력
print(f"// Verilog get_coef(idx) values, idx=0 (outermost) -> idx={len(side_indices) - 1} (innermost):")
for i, idx in enumerate(reversed(side_indices)):
    val = coefs[idx]
    if val < 0:
        print(f"    {math.ceil(math.log2(len(side_indices)))}'d{i}:  get_coef = -16'sd{abs(val)};")
    else:
        print(f"    {math.ceil(math.log2(len(side_indices)))}'d{i}:  get_coef = 16'sd{val};")
